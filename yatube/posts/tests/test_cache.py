...
class TestCache(TestCase):
    ...
    def test_post_in_cache(self):
        ...
        response_before_deleting_post = self.guest_client.get(reverse("posts:index"))
        post_obj = Post.objects.get(pk=1)
        post_obj.delete()
        response_after_deleting_post = self.guest_client.get(reverse("posts:index"))
        self.assertEqual(
            response_before_deleting_post.content,
            response_after_deleting_post.content)
        cache.clear()
        response_after_clear_cache = self.guest_client.get(reverse("posts:index"))
        self.assertNotEqual(
